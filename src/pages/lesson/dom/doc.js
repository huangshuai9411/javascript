...
由图可见，${reference('document')} 对象下面引用着整个文档的根节点，也就是说，想要查找文档中的某些节点，就可以利用**这棵文档树**，得到我们想要的东西。
#### **DOM——查询**
...
- 万能大法：${reference('document.querySelector')} 和 ${reference('document.querySelectorAll')} 。顾名思义，这两个方法只用传递合法的 [CSS 选择器](/overview/css)即可获取到元素。前者返回匹配的第一个元素，后者返回所有匹配的元素集合。
...
> ${reference('document.querySelector')}和${reference('document.querySelectorAll')}的详细使用场景对比：
`;

const docString2 = `
#### **DOM 操作——修改**

学了选择 DOM 的方法有什么用呢？还记得 [综述 > CSS](/overview/css) 部分留过的实现 Chrome 浏览器官网静态页面的作业吧，我们实现的是纯展示状态，没有任何动态交互效果。而学完这一节，你的网站就可以“动态变化了”。

一、 找到我们学习 HTML 标签时建的 index.html 文件用编辑器打开，同时在浏览器上查看该文件。原代码如下：
`;
...
const docString6 = `
> 刷新 index.html 网页，在底部能看到新加的元素吗？新加元素的样式相信同学们都能读懂每一行代码。在 HTML 文档中，body, head, html 标签都是唯一的，因此可以不用 document.getElementByXXX 的方式来获取，直接用 document.body, document.head, document.documentElement（chrome浏览器，IE 的 document.rootElement，初学者建议忽略 IE） 即可。
...
const docString7 = `
#### **DOM CSS**
刚才的代码，其实是元素样式属性的操作，元素都有哪些样式属性呢？我们在控制台打印一下：${reference('document.body.style')}，看完后还好吗？在学习 CSS 的章节接触到的所有 CSS 属性，全部出现在了这里。只不过为了支持对象的点取值操作，改成了驼峰形式。很多属性通过合并、简化的形式使用如 border，也细分为了 borderTop、borderRight、borderLeft、borderBottom、borderBottomWidth 等等，所以千万不要死背。当我们需要做什么的时候，我们是知道的，自然就会去接触相应的知识点。在每个节点对象上，我们可以通过 style 属性访问到这些样式值，当然也可以通过重新赋值来修改样式。
